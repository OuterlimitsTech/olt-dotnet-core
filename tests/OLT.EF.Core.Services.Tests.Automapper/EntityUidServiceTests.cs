...
public class EntityUidServiceTests : BaseUnitTests
{
    ...
    public async Task SoftDelete()
    {
        using (var provider = BuildProvider())
        {
            ...
            Assert.NotNull(await service.GetAsync<PersonDetailDto>(new OltSearcherGetByUid<PersonEntity>(model.UniqueId.Value)));
        ...
        using (var provider = BuildProvider())
        {
            ...
            Assert.NotNull(service.Get<PersonDetailDto>(new OltSearcherGetByUid<PersonEntity>(model.UniqueId.Value)));
